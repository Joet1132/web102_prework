...
const totalGamesCard = GAMES_JSON.reduce( (acc) => {
    return acc + 1;}, 0);

gamesCard.innerHTML = `${totalGamesCard.toLocaleString()}`
...
function filterUnfundedOnly() {
    ...
    let notFundedEnough = GAMES_JSON.filter( (games) => {
        return games.pledged < games.goal;}
    );

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(notFundedEnough);

    console.log(notFundedEnough.length);
};
filterUnfundedOnly();
...
function filterFundedOnly() {
    ...
    let isFundedEnough = GAMES_JSON.filter( (games) => {
        return games.pledged > games.goal;}
    );

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(isFundedEnough);
    console.log(isFundedEnough.length);
};
filterFundedOnly();
...
function showAllGames() {
    ...
    let allGames = GAMES_JSON.filter( (games) => {
        return games;}
    );
...
const numUnfundedGames = GAMES_JSON.filter( (games) => {
...
for ${totalGamesCard} ${totalGamesCard === 1 ? "game" : "games"}. 
Currently, ${numUnfundedGames} ${numUnfundedGames === 1 ? "game" : "games"} remain unfunded.
`
